...
def note_f_max(prob):
    return np.unravel_index(np.argmax(prob), prob.shape)
def note_f_prob(prob):
    return np.unravel_index(np.random.choice(prob.size, p=np.ravel(prob)), prob.shape)
if args.notemax:
